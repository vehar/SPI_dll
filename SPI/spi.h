...
//#define IOCTL_DDK_GET_DRIVER_IFC CTL_CODE(FILE_DEVICE_UNKNOWN,0x0100,METHOD_BUFFERED,FILE_ANY_ACCESS) 
		//
...
#define IOCTL_SPI_CONFIGURE					 CTL_CODE(FILE_DEVICE_UNKNOWN,0x0200,METHOD_BUFFERED,FILE_ANY_ACCESS)
#define IOCTL_SPI_WRITEREAD					 CTL_CODE(FILE_DEVICE_UNKNOWN,0x0201,METHOD_BUFFERED,FILE_ANY_ACCESS)
#define IOCTL_SPI_ASYNC_WRITEREAD		     CTL_CODE(FILE_DEVICE_UNKNOWN,0x0202,METHOD_BUFFERED,FILE_ANY_ACCESS)
#define IOCTL_SPI_ASYNC_WRITEREAD_COMPLETE   CTL_CODE(FILE_DEVICE_UNKNOWN,0x0203,METHOD_BUFFERED,FILE_ANY_ACCESS)
#define IOCTL_SPI_SET_SLAVEMODE				 CTL_CODE(FILE_DEVICE_UNKNOWN,0x0204,METHOD_BUFFERED,FILE_ANY_ACCESS)
#define IOCTL_SPI_MANUAL_CS_CTRL			 CTL_CODE(FILE_DEVICE_UNKNOWN,0x0205,METHOD_BUFFERED,FILE_ANY_ACCESS)

#define IOCTL_SPI_FORCE_CSX   CTL_CODE(FILE_DEVICE_UNKNOWN,0x0206,METHOD_BUFFERED,FILE_ANY_ACCESS)

#define IOCTL_SPI_BT_FULL_DUPLEX       CTL_CODE(FILE_DEVICE_UNKNOWN,0x0207,METHOD_BUFFERED,FILE_ANY_ACCESS)

typedef struct {
    DWORD address;
	// See OMAP35XX_TPS659XX_TI_V1\omap35xx\INC\omap35xx_mcspi.h (CHCONF section) for config bit field values.
	// See OMAP35xx Technical Reference Manual for bit field explanations.
    DWORD config;
...
__inline HANDLE SPIOpen(LPCTSTR pSpiName)
{
    ...
    if (!DeviceIoControl(
        hDevice,CTL_CODE(FILE_DEVICE_UNKNOWN,0x0100,METHOD_BUFFERED,FILE_ANY_ACCESS), (VOID*)&DEVICE_IFC_SPI_GUID,
...
__inline BOOL SPIEnableChannel(HANDLE hContext) //Enable one channel and assert the ChipSelect
{
    DEVICE_CONTEXT_SPI *pContext = (DEVICE_CONTEXT_SPI *)hContext;
